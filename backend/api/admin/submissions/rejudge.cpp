...
auto AdminSubmissionRejudge = [](client_conn conn, http_request request, param argv) {
    ...
    mysqli_execute(
        mysql,
        "UPDATE submission SET judged = 0, status = %d, score = 0, result = \"{}\", time = %ld WHERE id in %s",
        Waiting, 
        time(NULL),
        ids.c_str()
    );
