...
auto ProblemsDataDownload = [](client_conn conn, http_request request, param argv) {
    ...
    std::ifstream fin("../problem/" + std::to_string(atoi(argv[0].c_str())) + "/data.zip", std::ios::binary);
    if (!fin.is_open()) system(("cd ../problem/" + std::to_string(atoi(argv[0].c_str())) + " && zip -r data.zip * > /dev/null 2>&1").c_str());
    fin.close();

    /** 打开文件 */
    fin.open("../problem/" + std::to_string(atoi(argv[0].c_str())) + "/data.zip", std::ios::binary);
